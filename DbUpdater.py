...
from secrets_local import db_port
from datetime import datetime, timezone


def readDB(dbname, request):
...
def updateDB(request):
    ...
    if 'datetime' in request.keys():
        if len(request['datetime']) != 16:
            request['datetime'] = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M")
